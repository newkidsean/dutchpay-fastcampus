...
const renderComponent = () => {
  ...
  return {
    dateInput, descInput, amountInput, payerInput, addButton, descErrorMessage, payerErrorMessage, amountErrorMessage, shareButton
  }
}
...
describe('비용 정산 메인 페이지', () => {
  ...
  describe('비용 추가 컴포넌트', () => {
    ...
    test('비용 추가에 필수적인 값을 입력하지 않고 추가 버튼 클릭 시 에러 메시지 노출', async () => {
    ...
    test('비용 추가에 필수적인 값들을 입력한 후 추가 버튼 클릭 시 저장에 성공', async () => {
      const { descInput, amountInput, payerInput, addButton, descErrorMessage, amountErrorMessage, payerErrorMessage } = renderComponent()
